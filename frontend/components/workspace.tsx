...
function Dashboard({ system, router }: { system: SystemStatus | null; router: ReturnType<typeof useRouter> }) {
  ...
  return <>
    ...
    <div className="dashboard-grid">
      <section className="panel hero-panel"><div className="panel-heading"><div><span className="section-kicker">Review workflow</span><h2>Inspect a structural image with the trained model</h2></div><span className="tag tag-green">Real inference</span></div><div className="hero-visual"><div className="scan-line" /><div className="bridge-visual"><div className="bridge-slab" /><div className="bridge-pier left" /><div className="bridge-pier right" /><span className="crack-mark mark-one" /><span className="crack-mark mark-two" /></div><div className="hero-overlay"><span>Review milestone</span><strong>Upload → segment → review</strong><small>Results are saved locally</small></div></div><div className="hero-footer"><div><span className="tiny-label">Checkpoint</span><strong>EXP001 best.pt</strong></div><div><span className="tiny-label">Task</span><strong>Crack instance segmentation</strong></div><button className="button button-primary" onClick={() => router.push("/inspection/new")}>Run inspection <ArrowUpRight size={15} /></button></div></section>
